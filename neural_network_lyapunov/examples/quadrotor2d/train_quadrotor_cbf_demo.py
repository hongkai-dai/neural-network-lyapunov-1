...
def simulate(dynamics_model: control_affine_quadrotor.ControlAffineQuadrotor2d, barrier_relu, x_star, c, u_lo, u_up, epsilon, x0, pos_des, T):
    """
    ...
    def compute_control(x):
        ...
        with torch.no_grad():
            ...
            for i in range(dhdx.shape[0]):
                prog.addLConstr(gurobipy.LinExpr((dhdx[i]@G).tolist(), u_var), sense=gurobipy.GRB.GREATER_EQUAL, rhs=(-epsilon * h - dhdx[i] @ f).item())
            # Add the cost that min (u-u_des)ᵀR(u-u_des) + 2(x-x_des)ᵀS(A(x-x_des)+Bu)
            cost = gurobipy.QuadExpr()
            cost.add((u[0]-u_des[0]) *(u[0]-u_des[0]) * R[0, 0] + (u[1]-u_des[1]) *(u[1]-u_des[1]) * R[1, 1] + 2 * (u[0]-u_des[0]) * (u[1]-u_des[1]) * R[0, 1])
    ...
    def nn_plant_dynamics(t, x):
        u = compute_control(x)
        with torch.no_grad():
            return dynamics_model.dynamics(torch.from_numpy(x), torch.from_numpy(u))
    ...
    result = scipy.integrate.solve_ivp(plant_dynamics, [0, T], x0, events=reach_goal, max_step=0.01)
    result_nn = scipy.integrate.solve_ivp(nn_plant_dynamics, [0, T], x0, events=reach_goal, max_step=0.01)
...
if __name__ == "__main__":
    ...
    # simulate(dynamics_model, barrier_relu, x_star, c, u_lo, u_up, epsilon, np.zeros((6,)), np.array([0, -0.05]), 10)
